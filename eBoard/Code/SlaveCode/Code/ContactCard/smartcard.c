...
SC_ATR SC_A2R,SC_A2R1,SC_A2R2,SC_A2R3;
u8 SC_ATR_Table[SC_ATR_LEN];
...
void SC_Init(USART_TypeDef* USARTn)
{
   ...
   if(USARTn==USART1)
   {
    ...
    SC_PowerCmd(USART1,Bit_RESET);//VCC=0
    ...
    USART_InitStructure.USART_BaudRate =(3600000*SC_BAUDRATE_D)/SC_BAUDRATE_F;
    ...
    /* Set RSTIN LOW */  
    SC_ResetCmd(USART1,Bit_RESET);
    SC_PowerCmd(USART1,Bit_SET);//VCC=1
		SC_DirCmd(USART1,DIR_RECEIVE);
		SC_ClkCmd(USART1,ENABLE);
   ...
   else if(USARTn==USART2)
   {
    ...
    SC_PowerCmd(USART2,Bit_RESET);//VCC=0
    ...
    /* Set RSTIN LOW */  
    SC_ResetCmd(USART2,Bit_RESET);
    SC_PowerCmd(USART2,Bit_SET);//VCC=1
		SC_DirCmd(USART2,DIR_RECEIVE);
		SC_ClkCmd(USART2,ENABLE);
   }
   else
   {
    /* Enable GPIOB, GPIOD and GPIOE clocks */
    ...
    SC_PowerCmd(USART3,Bit_RESET);//VCC=0
    ...
    /* Set RSTIN LOW */  
    SC_ResetCmd(USART3,Bit_RESET);
    SC_PowerCmd(USART3,Bit_SET);//VCC=1
	  SC_DirCmd(USART3,DIR_RECEIVE);
	  SC_ClkCmd(USART3,ENABLE);
   } 
}
...
//      if((SC_ByteReceive(USARTn,&locData, SC_Receive_Timeout)) == SUCCESS)
//      {
//        if(locData != 0xFF)
//        {
 ...
 void SC_ApduHander(USART_TypeDef *USARTn,SC_ADPU_Commands *SC_ADPU, SC_ADPU_Responce *SC_ResponceStatus)
{
  ...
  if((SC_ByteReceive(USARTn,&locData,SC_Reset_Timeout)) == SUCCESS)
  {
    while(locData==0x60) //NULL 0x60
    {
      locData=0;
      SC_ByteReceive(USARTn,&locData,SC_Receive_Timeout);
    }
  ...
  //ACK 
   else if(((locData & (u8)0xFE) == (((u8)~(SC_ADPU->INS)) & (u8)0xFE))||((locData & (u8)0xFE) == (SC_ADPU->INS & (u8)0xFE)))
    {
  ...
  if(SC_ResponceStatus->SW1 == 0x00)
  {
    ...
    if (SC_ADPU->LC)
    {
      for(i = 0; i < SC_ADPU->LC; i++)
      {
        SCData = SC_ADPU->Data[i];        
        SC_ByteSend(USARTn, SCData);			
      }
      /* Flush the USARTn DR */
      (void)USART_ReceiveData(USARTn);
